...
cout <<"chk inside abcd 2" << endl;
				...
				cout <<"chk inside abcd 3 " <<curr_state.tid <<" "<< curr_state.count << endl;
				cout <<"chk inside abcd 3 " <<next_state.tid <<" "<< next_state.count << endl;
				cout <<"chk inside abcd 3 " <<tid <<" "<<tld->insCount<<" "<<waited <<" "<< executed<< endl;
			PIN_LockClient();	
	    ...
	    // return;
			...
			for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
			{
				...
				for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
				{
					...
					if (!filter.SelectTrace(trace) && (img_name.find("boost") == std::string::npos))
        		return;
				...
				if ((event1.tid == event2.tid) && (event2.i_count - event1.i_count < window_size)&& (event2.i_count - event1.i_count > (window_size * -1)) && (!event1.islock) && (!event2.islock) && (event1.addr != event2.addr))
				{
					...
					for (std::deque<lockInfo>::iterator ad = lockAddr.begin(); ad != lockAddr.end(); ++ad)
					{
						...
						{ 
							...
							{
								if ((adl->start <= event1.tid && event1.tid <= adl->end) && !(adl->start <= event2.tid && event2.tid <= adl->end))
								{
									non_relaxable = true;
									break;
								}
								if ((adl->start <= event2.tid && event2.tid <= adl->end) && !(adl->start <= event1.tid && event1.tid <= adl->end))
								{
									non_relaxable = true;
									break;
								}
        ...
        for (std::deque<relax_element>::iterator ss = es_pre->sleep_set.begin(); ss != es_pre->sleep_set.end(); ++ss)
				{
				  if (ss->type == 'w' && es->event.type == 'r' && es_pre->event.type == 'r')
				  {
				    if ((isRace(*ss, es_pre->event) && isRace(*ss, es->event) && es->event.tid != es_pre->event.tid) && !(ss->tid == es->event.tid && es->event.i_count == ss->i_count))
				    {
				      ...
				      if (mid_race && apply)
				      {
				      cout <<"b4 initials push 1" << endl;
				      	bool found1 = false,found2 = false;
				      	std::deque<bt_state>::iterator es_temp, es_pre_temp;
				      	for (std::deque<lockInfo>::iterator ad = lockAddr.begin(); ad != lockAddr.end(); ++ad)
								{
									for (std::deque<lockedRegion>::iterator adl = ad->locked_region.begin(); adl != ad->locked_region.end(); ++adl)   
									{
										if (adl->tid == es_pre->event.tid && ( adl->start > es_pre->event.i_count || adl->end < es_pre->event.i_count) && es_pre > bt_table.begin())
										{
											es_pre_temp = es_pre - 1;
											while (es_pre_temp->event.i_count != adl->start || es_pre_temp->event.i_count != adl->tid)
											{
												if (es_pre_temp == bt_table.begin())
													break;
												es_pre_temp--;
											}
											if (es_pre_temp == bt_table.begin())	
										...
										if (adl->tid == es->event.tid && ( adl->start > es->event.i_count || adl->end < es->event.i_count) &&  es < bt_table.end())
										{
											es_temp = es + 1;
											while (es_temp->event.i_count != adl->end || es_temp->event.i_count != adl->tid)
											{
												if (es_temp == bt_table.end())
													break;
												es_temp++;
											}
											if (es_temp != bt_table.end())
								...
								cout <<"initials push 1" << endl;
								if (found1 && found2)
				      		es_pre_temp->initials.push_back(es_temp->event);
				      	
				      	else if (found1 && !found2)
				      		es_pre_temp->initials.push_back(es->event);	
				      	else if (!found1 && found2)	
				      		es_pre->initials.push_back(es_temp->event);
				      	else	
			...
			//total++;
						...
						if (!has_initial)
						{
							if (es_pre->initials.empty())
							{
								if ((!causallyPrecedes(es->event, es_pre->event)))
								{
								cout <<"b4 initials push 3" << endl;
									bool found1 = false,found2 = false;
				      	std::deque<bt_state>::iterator es_temp, es_pre_temp;
				      	for (std::deque<lockInfo>::iterator ad = lockAddr.begin(); ad != lockAddr.end(); ++ad)
								{
								cout <<"b4 initials push 3.1" << endl;
									for (std::deque<lockedRegion>::iterator adl = ad->locked_region.begin(); adl != ad->locked_region.end(); ++adl)   
									{
									cout <<"b4 initials push 3.2" << endl;
										if (adl->tid == es_pre->event.tid && ( adl->start > es_pre->event.i_count || adl->end < es_pre->event.i_count) && es_pre > bt_table.begin())
										{
										cout <<"b4 initials push 3.2.1" << endl;
											es_pre_temp = es_pre - 1;
											while (es_pre_temp->event.i_count != adl->start || es_pre_temp->event.i_count != adl->tid)
											{
												if (es_pre_temp == bt_table.begin())
										...
										if (adl->tid == es->event.tid && ( adl->start > es->event.i_count || adl->end < es->event.i_count) && es < bt_table.end())
										{
										cout <<"b4 initials push 3.2.2" << endl;
											es_temp = es + 1;
											while (es_temp->event.i_count != adl->end || es_temp->event.i_count != adl->tid)
											{
												if (es_temp == bt_table.end())
								...
								cout <<"initials push 3" << endl;
								if (found1 && found2)
				      	{
				      		cout <<"initials push 31" << endl;
				      		es_pre_temp->initials.push_back(es_temp->event);
				      	}
				      	else if (found1 && !found2)
				      	{
				      		cout <<"initials push 32" << endl;
				      		es_pre_temp->initials.push_back(es->event);	
				      	}	
				      	else if (!found1 && found2)	
				      	{
				      		cout <<"initials push 33" << endl;
				      		es_pre->initials.push_back(es_temp->event);
				      	}	
				      	else	
				      	{
				      		cout <<"initials push 34" << endl;
				        	es_pre->initials.push_back(es->event);
				        }		
				        cout <<"after initials push 3" << endl;
				        }	
							}
							else
							{
			...
			while (std::getline(file1, subs))
				...
				if (!first_run)
				{
					...
					for (int i = bt_prev.size() - 1; i >= -1; i--)
					{
						...
						//cout << "event  " << rx.tid<<" "<< rx.i_count << endl;
	            ...
	            /*check if it has active relax*/
						while (!bt_prev[i].relax_event.empty())
						{
							...
							for (std::deque<relax_info>::iterator it = relax_ds.begin(); it != relax_ds.end(); ++it)
							{
								if ((it->tid1 == tid1) && (it->count1 == count1))
								{
									cout << "onw "<< ri.tid1 << " " << ri.count1 << " " << ri.tid2 << ri.count2 << endl;
